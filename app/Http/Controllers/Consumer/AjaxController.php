...
class AjaxController extends Controller
{
    ...
    public function buyNowAction(Request $request, $id)
    {
        ...
        if ($res->coid > 0) {
            ...
            if ($userType == 1) {
                $proObj->createAdminNotification($userId, $userType, $notifyMessage, $notifyType, $typeId);
            }

            if ($userType == 2) {
                $proObj->createUserNotification($userId, $userType, $notifyMessage, $notifyType, $typeId);
            }
            ...
            //  consumers
            //  'shop/product/buynow/' . $proslug
            return redirect(url()->previous())->with('error', 'Error occurred. Please try again later...!');
        }
    }
    ...
    {
        $proObj = new ConsumerController();
        $brands = $proObj->getProductCompanylist();
        return view('consumer.products.companylist')->with('data', $brands);
        // echo json_encode($products);
        // exit();
    }
    ...
    public function getSearchProductCompany($slug)
    {
        ...
        $products = $proObj->getSearchProductCompany($slug);
