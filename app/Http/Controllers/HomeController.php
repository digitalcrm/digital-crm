...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['products'] = $products->getProductsList($id);
    ...
    public function userCurrency($id)
    {
        $userdata = User::find($id);
        ...
        $data['croptions'] = $croptions;
        return view('auth.user.editcurrency')->with('data', $data);
    }

    public function userCurrencyUpdate(Request $request, $id)
    {
        // echo json_encode($request->input());
        $currency = $request->input('currency');
        $userdata = User::where('id', $id)->update(['cr_id' => $currency]);
        if ($userdata) {
            return redirect('/user/currency/' . $id)->with('success', 'Updated Successfully');
        } else {
            return redirect('/user/currency/' . $id)->with('error', 'Failed. Try again later...');
        }
    }
}
