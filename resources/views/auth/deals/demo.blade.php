@extends('layouts.adminlte-boot-4.user')
@section('content')

<!-- <link rel="stylesheet" type="text/css" href="https://netdna.bootstrapcdn.com/bootstrap/3.2.0/css/bootstrap.min.css"> -->
<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/dragula/3.7.2/dragula.min.css">
<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/dragula/3.7.2/dragula.min.js"></script>

<style id="compiled-css" type="text/css">
    .gu-mirror {
        filter: alpha(opacity=80);
        transform: skewX(-5deg) skewY(5deg);
    }

    .kanban-card {
        cursor: pointer;
    }

.info-box-number {
    display: contents;
    font-weight: 400;
}

    .horizontal-scrollable {
        overflow-x: auto;
        white-space: nowrap;
    }

    .horizontal-scrollable>.col-3 {
        display: inline-block;
        float: none;
    }

   .callout a {
	color: #000;
	font-weight:500;
	text-decoration: none;
    }
	.callout a:hover {
		text-decoration:underline;
		color: #000;
	}
.callout, .card, .info-box, .mb-3, .my-3, .small-box {
    margin-bottom: 0.5rem!important;
}
  
</style>

<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-4">
                    <h1>Deals <small class="badge badge-secondary" id="total"></small></h1>
                </div>
                <div class="col-sm-8">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">
                            <a class="btn btn-sm  btn-primary mr-1 px-3" href="{{url('deals/create')}}"><i class="far fa-plus-square mr-1"></i>New Deal</a>
                            <a class="btn btn-sm btn-outline-secondary" href="{{url('deals/import/csv')}}"><i class="fas fa-upload"></i></a>
                            <a class="btn btn-sm btn-outline-secondary" href="{{url('deals/export/csv')}}"><i class="fas fa-download"></i></a>
                            <a class="btn btn-sm btn-outline-secondary" href="{{url('reports/deals')}}"><i class="fa fa-chart-pie"></i></a>
                            <a class="btn btn-sm btn-outline-secondary" href="{{url('deals/status/won')}}"><i class="fa fa-thumbs-up"></i></a>
                            <a class="btn btn-sm btn-outline-secondary" href="{{url('deals/status/lost')}}"><i class="fa fa-thumbs-down"></i></a>
                            <button type="button" class="btn btn-outline-secondary" data-toggle="modal" data-target="#exampleModal2">
                                <i class="fa fa-filter"></i>
                            </button>
                            <a class="btn btn-sm btn-outline-secondary" href="{{url('deals/table/view')}}">Table</a>
                        </li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>

    <!-- Main content -->
    <section class="content mx-0">
        <div class="container-fluid">
            <!-- Small cardes (Stat card) -->
            <div class="row">
                <div class="col-lg-12">
                    @if(session('success'))
                    <div class='alert alert-success'>
                        {{session('success')}}
                    </div>
                    @endif

                    @if(session('error'))
                    <div class='alert alert-danger'>
                        {{session('error')}}
                    </div>
                    @endif

                    @if(session('info'))
                    <div class='alert alert-warning'>
                        {{session('info')}}
                    </div>
                    @endif
                    <div class="card shadow card-primary card-outline">
                        <!-- <div class="card-header">
                            <h3 class="card-title">Deals</h3>
                        </div> -->
                        <!--/.card-header-->
                        <div class="card-body" id="table">
                            <div id="kanban" class="row">
                                {!!$data['stageKanban']!!}
                                <!-- <div class="col-md-4">
                                    <div class="card card-default">
                                        <div class="card-header kanban-card">
                                            <h3 class="card-title">Todo</h3>
                                        </div>
                                        <div class="card-body">
                                            <div id="todo">
                                                &nbsp;
                                                <div id="card1" class="kanban-card" role="alert">
                                                    <div class="callout callout-success">
                                                        <h5>Card 1</h5>
                                                    </div>
                                                </div>
                                                <div id="card2" class="kanban-card" role="alert">
                                                    <div class="callout callout-success">
                                                        <h5>Card 2</h5>
                                                    </div>
                                                </div>
                                                <div id="card3" class="kanban-card" role="alert">
                                                    <div class="callout callout-success">
                                                        <h5>Card 3</h5>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="card card-default">
                                        <div class="card-header kanban-card">
                                            <h3 class="card-title">In Progress</h3>
                                        </div>
                                        <div class="card-body">
                                            <div id="in-progress">
                                                &nbsp;
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="card card-default">
                                        <div class="card-header kanban-card">
                                            <h3 class="card-title">Done</h3>
                                        </div>
                                        <div class="card-body">
                                            <div id="done">
                                                &nbsp;
                                            </div>
                                        </div>
                                    </div>
                                </div> -->
                            </div>
                        </div>
                        <!-- /.card-body -->
                       
                    </div>
                    <!-- /.card -->
                </div>
            </div>
            <!-- /.row -->

    </section>
    <!-- /.content -->
</div>
<!-- /.content-wrapper -->

<script type="text/javascript">
    // alert(stageIds);
    window.onload = function() {
        var stageIdsArr = [];
        $(".dlstage").each(function() {
            // alert($(this).attr('id'));
            var stId = "#" + $(this).attr('id');
            stageIdsArr.push(document.querySelector(stId));
        });

        // var dragulaCards = dragula([
        //     // document.querySelectorAll("#Confirm,#Negotiating,#Ready_To_Pay,#Order_Taken,#Won,#Lost")
        //     document.querySelector('#Confirm'),
        //     document.querySelector('#Negotiating'),
        //     document.querySelector('#Ready_To_Pay'),
        //     document.querySelector('#Order_Taken'),
        //     document.querySelector('#Won'),
        //     document.querySelector('#Lost')
        // ]);

        var dragulaCards = dragula(stageIdsArr);
        dragulaCards.on('drop', function(el, target, source, sibling) {
            // console.log(source); // from
            // console.log(target); // to
            // console.log(sibling); // next card

            // alert('element ' + el.id);
            // alert('source ' + source.id);
            // alert('target ' + target.id);

            changeDealStage(el.id, target.id, source.id);

        });

        var dragulaKanban = dragula([
            document.querySelector('#kanban')
        ], {
            moves: function(el, container, handle) {
                return handle.classList.contains('card-title');
            }
        });


    }
</script>

<script>
    var deleteAllUrl = "{{url('deals/deleteAll/{id}')}}";
    var dealFilter = "{{url('deals/filter/{type}/{start}/{end}')}}";
    var changedealstagedragndrop = "{{url('deals/kanban/changedealstagedragndrop/{deal_id}/{sfun_id}/{from_id}')}}";

    var viewdealstypesessionUrl = "{{url('deals/viewdealstypesession/{type}')}}";

    $(function() {

        $(".sidebar-menu li").removeClass("active");
        $("#lideals").addClass("active");

        // viewdealstypesession('kanban');

        $("#selectAll").click(function() {
            var checked = ($(".checkAll").prop('checked') == false) ? true : false;
            $(".checkAll").prop('checked', checked);
        });


        $("#close_reservation").click(function() {
            // alert("close_reservation");
            // $("#reservation").val("");
            var filter = $("#dealStageFilter").val();
            // alert(filter);

            getDeals(filter, '', '');

            $('#reservation').daterangepicker({
                locale: {
                    format: 'DD-MM-YYYY'
                },
                startDate: moment().subtract(30, 'days'),
                endDate: moment(),
                function(startDate, endDate) {
                    $('#reportrange span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'))
                }
            });

        });

        $("#dealStageFilter").change(function() {
            //            alert($(this).val());
            var filter = $(this).val();
            var startDate = $('#reservation').data('daterangepicker').startDate.format('DD-MM-YYYY');
            var endDate = $('#reservation').data('daterangepicker').endDate.format('DD-MM-YYYY');

            getDeals(filter, startDate, endDate);

            // $.get(dealFilter, {
            //     'type': filter,
            //     'start': startDate,
            //     'end': endDate,
            // }, function(result, status) {

            //     // alert(result);
            //     var res = eval("(" + result + ")");
            //     $("#total").text(res.total);
            //     $("#table").html(res.table);
            //     $('#dealsTable').DataTable({
            //         'paging': true,
            //         'lengthChange': true,
            //         'searching': true,
            //         'ordering': false,
            //         'info': true,
            //         'autoWidth': false,
            //         'columnDefs': [{
            //             type: 'date-uk',
            //             targets: 4
            //         }]
            //     });
            // });
        });

        $('#reservation').daterangepicker({
            locale: {
                format: 'DD-MM-YYYY'
            },
            startDate: moment().subtract(30, 'days'),
            endDate: moment(),
            function(startDate, endDate) {
                $('#reportrange span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'))
            }
        });

        $('#reservation').change(function() {
            var startDate = $('#reservation').data('daterangepicker').startDate.format('DD-MM-YYYY');
            var endDate = $('#reservation').data('daterangepicker').endDate.format('DD-MM-YYYY');
            var filter = $("#dealStageFilter").val();
            // alert(startDate + ' ' + endDate);
            // return false;

            getDeals(filter, startDate, endDate);

        });
    });

    function getDeals(filter, start, end) {

        $.get(dealFilter, {
            'type': filter,
            'start': start,
            'end': end,
        }, function(result, status) {

            // alert(result);
            var res = eval("(" + result + ")");
            $("#total").text(res.total);
            $("#table").html(res.table);
            $('#dealsTable').DataTable({
                'paging': true,
                'lengthChange': true,
                'searching': true,
                'ordering': false,
                'info': true,
                'autoWidth': false,
                'columnDefs': [{
                    type: 'date-uk',
                    targets: 4
                }]
            });
        });
    }

    function deleteAll() {
        var deleteIdlength = $('.checkAll:checked').length;
        if (deleteIdlength > 0) {
            var checkList = $('.checkAll:checked');
            var itemIds = [];
            $(checkList).each(function(index) {
                itemIds[index] = $(checkList).get(index).id;
            });

            // alert(itemIds);

            $.get(deleteAllUrl, {
                'id': itemIds
            }, function(result, status) {
                if (result > 0) {
                    alert('Deleted successfully...');
                } else {
                    alert('Failed. Try again...');
                }
                location.reload();
            });
        } else {
            alert('Please select...');
        }
    }

    function changeDealStage(deal_id, sfun_id, from_id) {

        // alert(deal_id + ' ' + sfun_id);
        // alert(changedealstagedragndrop);
        $.get(changedealstagedragndrop, {
            'deal_id': deal_id,
            'sfun_id': sfun_id,
            'from_id': from_id,
        }, function(result, status) {

            // alert(result);
            var res = eval("(" + result + ")");
            var fromStageId = res.fromStageId;
            var toStageId = res.toStageId;

            // alert(fromStageId + ' ' + toStageId);

            var fromAmount = res.fromAmount;
            var toAmount = res.toAmount;

            $("#" + fromStageId).html(fromAmount);
            $("#" + toStageId).html(toAmount);
        });
    }


    // function viewdealstypesession(type) {
    //     $.get(viewdealstypesessionUrl, {
    //         'type': type
    //     }, function(result, status) {
    //         alert(result);
    //     });
    // }
</script>
@endsection